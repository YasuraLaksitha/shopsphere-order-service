    ...
    public void handlePaymentEvent(Event event) {
        ...
        event.getDataObjectDeserializer().getObject().ifPresent(stripeObject -> {
            ...
            if (stripeObject instanceof Session session) {
                ...
                } else if (event.getType().equals("payment_intent.payment_failed") || event.getType().equals("checkout.session.async_payment_failed") || event.getType().equals("charge.failed")) {
                    orderService.updateOrderStatus(orderId, OrderStatus.PAYMENT_FAILED.name());
                }
            }
        });
    }
